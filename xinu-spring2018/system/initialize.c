...
struct xts_tab xts_conf[XTSSIZE];	  /* table of priorities */
struct xts_multifb xts_ready[XTSSIZE]; /* table of ready list */
qid16 xts_arr[XTSSIZE];				   /* arr list */
...
static void sysinit()
{
	...
	for (int i = 0; i < XTSSIZE; i++)
	{
		...
		kprintf("quantum: %d\ttqexp: %d\tslpret: %d\n", xts_conf[i].xts_quantum,
				xts_conf[i].xts_tqexp, xts_conf[i].xts_slpret);
	}
	...
	/* lab4(Han Wang): create TS ready list */
	kprintf("debug1\n");
	for (int i = 0; i < 60; i++)
	{
		xts_arr[i] = newqueue();
		xts_ready[i].status = 0;
		xts_ready[i].queue_head = queuehead(xts_arr[i]);
		xts_ready[i].queue_tail = queuetail(xts_arr[i]);
	}
	kprintf("debug2\n");
